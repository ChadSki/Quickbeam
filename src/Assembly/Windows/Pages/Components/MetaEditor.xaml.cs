...
namespace Assembly.Windows.Pages.Components
{
    ...
    public partial class MetaEditor : UserControl
    {
            ...
            ICacheFile cache, IStreamManager streamManager, IRTEProvider rteProvider, Trie stringIDTrie)
        {
            ...
            // Load Plugin Path
            string className =
                VariousFunctions.SterilizeTagClassName(CharConstant.ToString(tag.RawTag.Class.Magic)).Trim();
        ...
        private void RevisionViewer()
        {
            if (_pluginVisitor != null && _pluginVisitor.PluginRevisions != null)
                MetroPluginRevisionViewer.Show(_pluginVisitor.PluginRevisions,
                    CharConstant.ToString(_tag.RawTag.Class.Magic));
            else
...
#else
                    var metaUpdate = new MetaWriter(stream, (uint) _tag.RawTag.MetaLocation.AsOffset(), _cache,
                        _buildInfo, type,
            ...
            else if (_rteProvider != null)
            {
                using (IStream metaStream = _rteProvider.GetMetaStream(_cache))
                {
                    if (metaStream != null)
                    {
                        ...
                        var metaUpdate = new MetaWriter(metaStream, _tag.RawTag.MetaLocation.AsPointer(), _cache,
                            _buildInfo, type,
        ...
        public void LoadNewTagEntry(TagEntry tag)
        {
            ...
            // Load Plugin Path
            string className =
                VariousFunctions.SterilizeTagClassName(CharConstant.ToString(_tag.RawTag.Class.Magic)).Trim();
            _pluginPath = string.Format("{0}\\{1}\\{2}.xml", VariousFunctions.GetApplicationLocation() + @"Plugins",
                _buildInfo.Settings.GetSetting<string>("plugins"), className);
